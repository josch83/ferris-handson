...
class UserAccountService:
    def __init__(self, token_passed=None):
        host = os.environ.get("JSCTZI_API_HOST")
        port = os.environ.get("JSCTZI_API_PORT")

        uri = f"{host}:{port}/"

        config = jsctzi_api_client.Configuration()
        config.host = uri
        if token_passed:
            config.api_key["Authorization"] = token_passed
            config.api_key_prefix["Authorization"] = "Bearer"
        self.user_account_api_instance = jsctzi_api_client.UserAccountApi(jsctzi_api_client.ApiClient(config))
