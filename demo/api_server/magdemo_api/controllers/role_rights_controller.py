...
import traceback
import connexion
import magdemo_api.models.role_rights as swagger  # noqa: E501
from magdemo_api.models.api_classes import db
from magdemo_api.models.sqla_models import (
    role_access,
    RoleRights,
)  # noqa: E501
...
def get_role_rightss(
    ...
    try:
        ...
        if equal_entity:
            query = query.filter(RoleRights.entity == equal_entity)
        if not_entity:
            query = query.filter(RoleRights.entity != not_entity)
        if in_list_entity:
            query = query.filter(RoleRights.entity.in_(in_list_entity))
        if isnull_entity:
