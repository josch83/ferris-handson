...
from ..utils import (
    MAGDEMOGenericColumn,
    MAGDEMOGenericInterface,
    MAGDEMOGenericSession,
    create_logger,
    get_access_token,
...
class UserAccountModel(FerrisGenericModel):

    changed_by = MAGDEMOGenericColumn(
        str,
        nullable=True,
        foreign_key=False,
    )
    changed_on = MAGDEMOGenericColumn(
        datetime.datetime,
        nullable=True,
        foreign_key=False,
    )
    created_by = MAGDEMOGenericColumn(
        str,
        nullable=True,
        foreign_key=False,
    )
    created_on = MAGDEMOGenericColumn(
        datetime.datetime,
        nullable=True,
        foreign_key=False,
    )
    fullname = MAGDEMOGenericColumn(
        str,
        nullable=True,
        foreign_key=False,
    )
    id = MAGDEMOGenericColumn(
        int,
        primary_key=True,
        nullable=True,
    )
    name = MAGDEMOGenericColumn(
        str,
        nullable=True,
        foreign_key=False,
    )
    addresses = MAGDEMOGenericColumn(
        int,
        nullable=False,
        foreign_key=True,
        foreign_key_class="AddressModel",
        relationship_type=FerrisGenericRelationshipType.MANY_TO_MANY,
    )
    ...
    def to_dict(self):
        ...
        for col_name in self.columns:
            if "<fab_addon_magdemo_api.utils.MAGDEMOGenericColumn" in repr(
...
class UserAccountSession(MAGDEMOGenericSession):
    def __init__(self) -> None:
        super().__init__(UserAccountModel, ServiceFactory(service=services_available.USER_ACCOUNT), logger)
...
class UserAccountInterface(MAGDEMOGenericInterface):
    def __init__(self, obj, session) -> None:
        super().__init__(obj, session, ServiceFactory(service=services_available.USER_ACCOUNT), logger)
