...
from flask_appbuilder.tests.sqla.models import (
    Model1,
    Model2,
    Model3,
    ModelWithEnums,
...
class MVCTestCase(BaseMVCTestCase):
    def setUp(self):
        ...
        class Model2DirectByChartView(DirectByChartView):
            ...
            definitions = [{"group": "field_string", "series": ["field_integer", "field_float"]}]
        ...
        self.appbuilder.sm.add_user(USERNAME_ADMIN, "admin", "user", "admin@fab.org", role_admin, PASSWORD_ADMIN)
    ...
    def test_auth_builtin_roles(self):
        ...
        rv = client.get("/model1view/show/" + str(model.id))
        self.assertEqual(rv.status_code, 200)
        # Test unauthorized EDIT
        rv = client.get("/model1view/edit/" + str(model.id))
        self.assertEqual(rv.status_code, 302)
        # Test unauthorized DELETE
        rv = client.get("/model1view/delete/" + str(model.id))
        self.assertEqual(rv.status_code, 302)
    ...
    def test_model_crud_edit(self):
        ...
        model = self.appbuilder.get_session.query(Model1).filter_by(field_string=field_string).one_or_none()
    ...
    def test_model_crud_composite_pk(self):
        ...
        model = self.appbuilder.get_session.query(Model3).filter_by(pk1="2", pk2="2017-02-02 00:00:00").one_or_none()
    ...
    def test_modelview_add_redirects(self):
        ...
        rv = client.post("/model1viewwithredirects/add", data=dict(field_string="test_redirect"))
    ...
    def test_method_permission_override(self):
        ...
        self.assertEqual(rv.status_code, 200)
        model1 = (
            self.appbuilder.get_session.query(Model1).filter_by(field_string=f"test{MODEL1_DATA_SIZE+1}").one_or_none()
        )
        self.assertIsNotNone(model1)
        ...
        # Unauthorized delete
        model1 = self.appbuilder.get_session.query(Model1).filter_by(field_string=f"test1").one_or_none()
        pk = model1.id
    ...
    def runTest(self):
        ...
        self.test_model_crud_add()
        self.test_model_crud_edit()
        self.test_index()
        self.test_sec_login()
        self.test_auth_builtin_roles()
        self.test_sec_reset_password()
        self.test_generic_interface()
        self.test_add_excluded_cols()
        self.test_model_add_unique_validation()
        self.test_model_edit_unique_validation()
        self.test_model_edit_required_validation()
        # self.test_class_method_permission_override()
        # self.test_permission_converge_compres()


if __name__ == "__main__":
    test_config.set_test_envs()
